...
wget -q https://github.com/$(wget -qO- https://github.com/notepad-plus-plus/notepad-plus-plus/releases | grep download/ | cut -d '"' -f2 | sed -n 7p)
./7zz x -aos "npp.$ver.Installer.x64.exe" -x'!change.log' -x'!doLocalConf.xml' -x'!LICENSE' -x'!NppShell_06.dll' -x'!readme.txt' -x'!userDefinedLang-markdown.default.modern.xml' -o"npp-stable/usr/share/notepad-plus-plus" &>/dev/null
# winedata
...
cp -R $appdir/'$_23_'/* "npp-stable/winedata/Application Data/Notepad++";cp -R $appdir/'$_24_'/* "npp-stable/usr/share/notepad-plus-plus/plugins";cp -R $appdir/'$_26_'/* "npp-stable/usr/share/notepad-plus-plus/plugins/Config"
find $appdir/'$PLUGINSDIR' -type f -name '*.xml' -print0 | while read -d $'\0' file; do cp -v "$file" $appdir/localization/ &>/dev/null; done
rm -R $appdir/'$_23_';rm -R $appdir/'$_24_';rm -R $appdir/'$_26_';rm -R $appdir/'$PLUGINSDIR';
